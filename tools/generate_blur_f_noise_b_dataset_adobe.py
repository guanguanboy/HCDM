...
for sub_dataset_name in sorted(sub_dataset_list):
    ...
    comp_file_path = sub_dataset_name + '/composite_images_test_samll/' #原始合成图像路径
    comp_file_list = os.listdir(comp_file_path) #获取合成图像的list

    mask_file_path = sub_dataset_name + '/masks/' #原始合成图像mask路径

    for image in sorted(comp_file_list):
        ...
        #生成带噪图的存储文件夹
        save_noisy_img_path = sub_dataset_name + '/composite_images_test_blured_f_noisy25_b_istropic_small/'
