...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # patch merging layer
        if downsample is not None: # downsample是一个PatchEmbed操作，其patch_size为2，因此刚好可以实现2倍的下采样。
...
class FocalAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_feats, img_size, patch_size):
        super(FocalAttention, self).__init__()

        self.img_size = img_size
        self.patch_size = patch_size
        self.patch_embed = PatchEmbed(img_size=to_2tuple(img_size), patch_size=patch_size, in_chans=num_feats, embed_dim=embed_dim)

        self.focal_block = FocalTransformerBlock(dim=embed_dim, input_resolution=(img_size//patch_size,img_size//patch_size), num_heads=3)  #56 = 224/window_size,window_size的值为4

        self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim, num_feats, 3, 1, 1), nn.LeakyReLU(inplace=True))

        self.upsample = Upsample(patch_size, num_feats)
                                                           
    def forward(self, x):
        patch_embed = self.patch_embed(x)
        focal_block = self.focal_block(patch_embed)
            #将3维tensor变形为4维的tensor。
        out = focal_block.view(focal_block.shape[0], self.img_size//self.patch_size, self.img_size//self.patch_size, -1).permute(0, 3, 1, 2).contiguous()
...
def test_focal_transformer_block():
    img_size = 128
    patch_size = 4
    x = torch.rand(1, 3, img_size, img_size).cuda()

    patch_embed = PatchEmbed(
            img_size=to_2tuple(img_size), patch_size=patch_size, in_chans=3, embed_dim=96).cuda()

    #x = torch.rand(1,3136, 96).cuda() #96是embed_dim=96
    focal_block = FocalTransformerBlock(dim=96, input_resolution=(img_size//patch_size,img_size//patch_size), num_heads=3).cuda()  #56 = 224/window_size,window_size的值为4

    embed_output = patch_embed(x)
    output = focal_block(embed_output)
    print('focal block output shape =', output.shape) #torch.Size([1, 3136, 96]) 

    #将3维tensor变形为4维的tensor。
    x = output.view(output.shape[0], img_size//patch_size, img_size//patch_size, -1).permute(0, 3, 1, 2).contiguous()
    print("4 dimension tensor output shape =", x.shape) # torch.Size([1, 96, 56, 56])  

    #使用恢复原来的shape
    conv_before_upsample = nn.Sequential(nn.Conv2d(96, 3, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True)).cuda()
    before_upsample = conv_before_upsample(x)
    print('before upsample shape:', before_upsample.shape)
    upsample = Upsample(patch_size, 3).cuda() 

    after_upsample = upsample(before_upsample)
    print('after upsample shape:', after_upsample.shape)

        
def test_focal_attention():
    img_size = 224
    x = torch.rand(3, 512, 8, 8).cuda()
    attn = FocalAttention(embed_dim=96, num_feats=512, img_size=8, patch_size=4).cuda()

    output = attn(x)
    print('test focal attn shape =', output.shape) #torch.Size([3, 512, 8, 8])
...
if __name__ == '__main__':
    ...
    #profile(model, x)
    output = model(x)
    print('focal transformer output.shape = ', output.shape) #torch.Size([16, 1000])

    test_focal_transformer_block()

    test_focal_attention()
