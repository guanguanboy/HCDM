...
#from .focal_attention import FocalAttention
import models.guided_diffusion_modules.focal_attention as fa
        ...
        for level, mult in enumerate(channel_mults): #channel乘数[值分别为1，2，4，8]
            for _ in range(res_blocks): #res_blocks == 2
                ...
                if ds in attn_res: #attn_res=[8] 第四次for判断的时候ds的值会是8，会走这个分支
                    ...
                    layers.append(
                        fa.FocalAttention(embed_dim=96, num_feats=ch, img_size=image_size//ds, patch_size=4)
