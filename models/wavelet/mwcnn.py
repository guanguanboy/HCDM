...
class UNet(nn.Module):
    def __init__(self, conv=common.default_conv):
        ...
        m_head = [common.BBlock(conv, 24, 160, 3, act=act)] #这里将12修改为4就可以处理灰度图了。
        d_l1 = []
        for _ in range(n):
    ...
    def forward(self, x):
        ...
        x = self.final(x) #为了满足unet 输出通道的限制，这里是我加的。
        return x
...
def test_mwcnn():
    mwcnn_model = UNet().cuda() #当前BSR只能对灰度图进行处理
    input_tensor = torch.randn(5, 6, 224, 224).cuda()

    output = mwcnn_model(input_tensor)
    print(output.shape) #torch.Size([5, 3, 224, 224])
