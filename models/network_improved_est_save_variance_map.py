...
variance_img_path = '/data1/liguanlin/Datasets/iHarmony/Hday2night/variance_images/'
#global varinace_img_count
varinace_img_count = 1
...
class Network(BaseNetwork):
    ...
    @torch.no_grad()
    def p_sample(self, y_t, t, i, clip_denoised=True, y_cond=None, mask=None): #从y_t采样t时刻的重构值
        ...
        if i == 700:
            global varinace_img_count
            varinace_img_count += 1
            filename = variance_img_path + str(varinace_img_count) + '.jpg'
            save_image_tensor(out["log_variance"], filename)
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8): #采样过程，类似于IDDPM中的源码p_sample_loop
        ...
        for i in tqdm(reversed(range(0, self.time_step_respacing)), desc='sampling loop time step', total=self.time_step_respacing):
            ...
            y_t = self.p_sample(y_t, t, i, y_cond=y_cond, mask=mask) #将y_t作为下一个迭代的输入来生成新的y_t #会在p_sample调用函数。
            if mask is not None:
